...
public class RunIterator implements Iterator {
  ...
  // the encoding.
  RunIterator(DListNode head,int size) {
	  this.node=head;
	  this.size=size;
	  count=1;
  ...
  public int[] next() {
	 ...
	 try{
		 result[0]=((record)(node.item())).runlength;
	     result[1]=(short)((record)(node.item())).pixel.red.intensity;
	     result[2]=(short)((record)(node.item())).pixel.green.intensity;
	     result[3]=(short)((record)(node.item())).pixel.blue.intensity;
	     this.node=(DListNode)node.next();
